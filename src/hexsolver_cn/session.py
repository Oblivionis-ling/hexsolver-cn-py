...
class InteractivePuzzleSession:
    ...
    def undo(self) -> Optional[StateChange]:
        ...
        if cell is None:
            raise BoardStateError(f"撤销时找不到格子 {change.coord}。")
        if self.board.remaining_blue is not None:
            if change.after is CellVisualType.BLUE and change.before is not CellVisualType.BLUE:
                self.board.remaining_blue += 1
            elif change.after is not CellVisualType.BLUE and change.before is CellVisualType.BLUE:
                self.board.remaining_blue -= 1
        cell.visual_type = change.before
