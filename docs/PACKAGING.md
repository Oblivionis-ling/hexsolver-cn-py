...
`0.5.0` 将现有 PySide6 求解器封装为一个 Windows x64 EXE。打包层不重写界面、生成器或求解器；源码版与成品版都从 `MainWindow`、`HexReasoningSolver` 和同一组 Easy/Hard 后端启动。
...
2026-08-05 已验证发布成品：96,461,270 字节（91.99 MiB），SHA-256 为 `2628f23b6e11888e6613aa64d654d9016c1679c7ed8cd59af7c7e9c24c1db7da`。
...
截图入口当前关闭，因此 0.5.0 成品排除 OpenCV、ONNX Runtime 和 RapidOCR，以减少体积；这不改变当前可用 UI 或种子求解流程。
