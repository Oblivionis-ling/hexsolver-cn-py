...
从 [GitHub Release v0.7.1](https://github.com/Oblivionis-ling/hexsolver-cn-py/releases/tag/v0.7.1) 下载：

- `HexInfiniteSolver-0.7.1-windows-x64.exe`
- `HexInfiniteSolver-0.7.1-windows-x64.exe.sha256`

无需安装 Python、Conda 或项目依赖，下载后直接双击 EXE。`0.7.1` 沿用同一套生成器、求解器和种子缓存核心；推理引用悬停时不再弹出提示框，棋盘联动改为轻薄柔光与精细轮廓组成的分层高亮，点击后仍可固定并加粗文字，再次点击取消。
...
![0.7.1 行线索引用与分层棋盘高亮](docs/design/ui-v071-reason-row-linked.png)

![0.7.1 坐标数组与分层棋盘高亮](docs/design/ui-v071-reason-array-linked.png)
...
普通用户优先使用 Release 中的 `HexInfiniteSolver-0.7.1-windows-x64.exe`，直接双击即可。
...
当前 66 项自动测试覆盖 TSV 合约、两套交错坐标相位、官方 Y 轴方向、Easy/Hard 样本、Unity RNG 位模式、默认后端不启动游戏、揭示/撤销、完整逐步回放、五类详细推理理由、单坐标/长数组/三方向行引用解析、Qt 悬停无提示框、分层预览/固定/清理联动、减少动态模式、三种手动标记轮廓的实际像素、长文本滚动显示、后台生成和 Qt 主流程。Easy/Hard seed 1–50 及两个八位边界种子已做原版逐字段差分；代表种子还会完整解到零未知格并逐步核对私有答案。
...
- `packaging/`：0.7.1 Windows 单文件构建、图标/版本资源和成品冒烟测试入口。

## 文档

- [开发历史](DEVELOPMENT_HISTORY.md)：从 `0.1.0` 截图原型到当前 `0.7.1` 版本。
